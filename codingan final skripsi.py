...
DEFAULT_OUTPUT_DIR = "ann_final_improved"
...
SEARCH_SPACE = {
    "batch_size": [64, 96],
    "learning_rate": [2e-5, 3e-5, 4e-5, 5e-5, 6e-5, 5.5e-5, 6.5e-5],
...
def evaluate_predictions(y_true: np.ndarray, y_pred: np.ndarray):
    ...
    return {
        ...
        "raw_mape": float(mean_absolute_percentage_error(y_true, y_pred_raw)),
        ...
        "clipped_mape": float(mean_absolute_percentage_error(y_true, y_pred_clipped)),
        ...
        "halfstep_mape": float(mean_absolute_percentage_error(y_true, y_pred_half)),
...
def run_multi_seed_evaluation(train_df_raw, val_df_raw, test_df_raw, config, eval_seeds, output_dir, max_epochs, patience, min_delta):
    ...
    for seed in eval_seeds:
        ...
        row = {
            ...
            "test_mape": float(test_metrics["clipped_mape"]),
...
def evaluate_best_trial_on_test(best_trial, best_data_bundle, output_dir, data_file, meta_info):
    ...
    print("=" * 80)
    print(f"Test MAPE clipped  : {test_metrics['clipped_mape']:.2f}%")
    print(f"Test MAPE halfstep : {test_metrics['halfstep_mape']:.2f}%")
    print(f"Test MAPE raw      : {test_metrics['raw_mape']:.2f}%")
